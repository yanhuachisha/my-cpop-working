...
def test_jay_instagram_unconfigured_falls_back_to_profile(monkeypatch):
    monkeypatch.delenv("JAY_INSTAGRAM_USER_ID", raising=False)
    monkeypatch.delenv("INSTAGRAM_ACCESS_TOKEN", raising=False)

    response = client.get("/api/jay/instagram")
    assert response.status_code == 200
    payload = response.json()

    assert payload["configured"] is False
    assert payload["profile_url"] == "https://www.instagram.com/jaychou/"
    assert payload["posts"] == []
    assert payload["sources"]


def test_jay_instagram_configured_returns_posts(monkeypatch):
                ...
                "data": [
                    {
                        ...
                        "timestamp": "2026-08-16T08:00:00+0000",
                    }
                ]
            }
    ...
    def fake_get(url, params, timeout):
        ...
        assert params["fields"] == "id,caption,media_type,media_url,permalink,timestamp"
        assert params["limit"] == 6
        assert timeout == 8.0
        return FakeResponse()

    monkeypatch.setenv("JAY_INSTAGRAM_USER_ID", "ig-user")
    ...
    assert payload["posts"][0]["media_url"] == "https://cdn.example/post.jpg"


def test_jay_special_page_has_timeline_and_graph():
    response = client.get("/api/jay")
    assert response.status_code == 200
    payload = response.json()
    assert payload["artist"]["id"] == "jay-chou"
